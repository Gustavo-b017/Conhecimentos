...
* **O Problema que Resolve:** No gerenciamento de cópias de segurança (Backups), copiar 1 Terabyte de dados todo dia pela rede exauriria a [[Rede_Largura_Banda]] e aumentaria drasticamente a [[Rede_Latencia]]. O `rsync` resolve isso sincronizando apenas as alterações diárias em segundos, economizando tráfego [[Rede_TCP_IP]] e reduzindo o RPO ([[Cyber_RPO]]).
*   **Visão Sênior (Vulnerabilidades/Escala):** O `rsync` é um executor cego ([[Mindset_Pensamento_Critico]]: ferramentas não pensam). Se a máquina principal sofrer um ataque de [[Cyber_Malware_Ransomware]] e os arquivos forem criptografados, o `rsync` verá isso como "arquivos modificados" e imediatamente sincronizará os arquivos destruídos por cima do seu backup limpo, arruinando sua redundância se você não possuir versionamento ou *snapshots* isolados.

#### 3. As Sinapses (Conexões Livres e Interdisciplinares)
O `rsync` atua como um **Editor de Livros Paranóico**. Se o autor alterar apenas uma vírgula na página 45 de um livro de 1.000 páginas, o comando tradicional imprimiria as 1.000 páginas novamente e jogaria a cópia velha no lixo. O `rsync` olha as 1.000 páginas, diz "só a 45 mudou", recorta o pedaço exato da página 45 com um estilete, cola na cópia de backup e encerra o trabalho. Isso é a aplicação técnica do [[Mindset_Essencialismo]]: mover apenas o necessário.
