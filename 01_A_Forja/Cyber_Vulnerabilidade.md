...
*   **Visão Sênior (Vulnerabilidades/Escala):** Vulnerabilidades são infinitas e inesgotáveis (novas [[Cyber_Framework_MITRE_ATTACK|CVEs]] ou [[Cyber_CVE|CVEs]] nascem diariamente). O erro primário de equipes juniores é tentar corrigir 100% delas, exaurindo a máquina. Ambientes seniores priorizam as correções baseando-se na existência de ameaças ativas ([[Cyber_Ameaca]]). A pior vulnerabilidade não está no software, mas no "[[Cyber_IoT_Security|Shadow IT]]" (dispositivos na rede que a equipe de TI desconhece).
...
```bash
# Varrer o IP alvo utilizando a biblioteca NSE ([[Ferramenta_Nmap|Nmap]] Scripting Engine) focada em vulnerabilidades conhecidas
