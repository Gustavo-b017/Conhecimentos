...
- **Como Funciona:** Ele atua na fronteira da Camada 7. Ele escuta as portas de rede (ex: 8080 TCP), intercepta a requisição HTTP bruta em formato de texto, empacota isso em objetos nativos (`HttpServletRequest`) e entrega para o código Java rodar a regra de negócio.
...
- **Visão Sênior (Vulnerabilidades/Escala):** O modelo clássico do Tomcat aloca pesadamente uma Thread de RAM do Sistema Operacional para cada requisição simultânea. Sob ataques de [[Cyber_Ataque_DDoS_SYN_Flood]] ou em picos de Black Friday, as threads esgotam rapidamente, causando *Crash*. Hoje, arquiteturas sêniores usam o Tomcat *Embarcado* direto na aplicação (Spring Boot) e rodam múltiplas réplicas orquestradas em Kubernetes, não dependendo de um servidor físico único.
