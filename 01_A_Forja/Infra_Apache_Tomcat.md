...
- **Como Funciona:** Ele atua na fronteira da Camada 7. Ele escuta as portas de rede (ex: 8080 [[Rede_TCP|TCP]]), intercepta a requisição [[Rede_HTTP|HTTP]] bruta em formato de texto, empacota isso em objetos nativos (`HttpServletRequest`) e entrega para o código Java rodar a regra de negócio.
...
O poder do ecossistema moderno de infraestrutura abstraída. Hoje, você não instala o Tomcat no [[OS_Linux|Linux]]. O [[Java_SpringBoot|Spring Boot]] carrega o servidor dentro de si via dependência [[Java_Maven|Maven]], levantando a infraestrutura dinamicamente:
```xml
