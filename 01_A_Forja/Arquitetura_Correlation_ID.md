...
**O que é:** O *Correlation ID* (ID de Correlação) é um token alfanumérico único (geralmente um UUID) gerado no instante em que a primeira requisição entra no ecossistema e que é compulsoriamente propagado pelos cabeçalhos de todos os saltos de rede e logs em uma arquitetura distribuída.

#### 2. A Desconstrução (Mecânica e Pontos de Falha)
- **Como Funciona:** O usuário clica em "Comprar". O [[Cyber_Firewall_WAF]] gera o ID `550e8400-e29b`. Esse ID é repassado no *header* HTTP para o API Gateway -> Serviço de Carrinho -> Kafka -> Serviço de Estoque. Cada vez que um desses sistemas emite um log ou uma mensagem de erro, eles escrevem o ID na frente do log.
...
- **Visão Sênior (Vulnerabilidades/Escala):** Injetar manualmente o Correlation ID em cada requisição *HTTP Client* do seu código polui a regra de negócio. Arquitetos utilizam padrões como o `MDC` (Mapped Diagnostic Context) no Java/SLF4J, ou delegam o rastreamento integralmente para ferramentas de Malha de Serviço (Service Mesh) e observabilidade como o *OpenTelemetry* e o *Jaeger*.
