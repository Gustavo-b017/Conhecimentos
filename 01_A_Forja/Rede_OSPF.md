...
O OSPF é a materialização do **Aplicativo Waze** rodando no cérebro do roteador. Protocolos velhos eram como perguntar a direção num posto de gasolina ("Vire à direita no próximo semáforo"). O OSPF baixa o mapa inteiro da cidade ([[Rede_Topologias]]) de uma vez só. Se houver um acidente na rua principal (Cabo rompido), o roteador não precisa perguntar a ninguém; ele instantaneamente roda a fórmula de Dijkstra na memória, traça a rota pelas vielas laterais e redireciona o tráfego em milissegundos sem derrubar as chamadas [[Rede_VoIP]]. É a base para tecnologias avançadas como o [[Rede_Flex_Algo]].
