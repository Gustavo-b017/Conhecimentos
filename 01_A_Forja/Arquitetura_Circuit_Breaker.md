...
A implementação moderna no ecossistema Java (Spring Boot) costuma utilizar a biblioteca *Resilience4j*. A configuração não suja a regra de negócio, ela apenas "envelopa" o método de chamada HTTP:
```java
...
public PagamentoDTO processarPagamento() {
    return restTemplate.getForObject("https://api.banco.com/pagar", PagamentoDTO.class);
}

// O Plano B executado instantaneamente enquanto o circuito está "Aberto"
public PagamentoDTO metodoPlanoB(CallNotPermittedException ex) {
    return new PagamentoDTO("Serviço indisponível. Seu dinheiro está seguro. Tente mais tarde.");
}
````
