...
*   **O Problema que Resolve:** Anula o acoplamento do cliente com a topologia do backend e centraliza *cross-cutting concerns* (Segurança, Rate Limiting, SSL Termination, Logs). Sem ele, cada um dos 50 microsserviços da empresa teria que programar a própria validação de JWT e sua própria segurança HTTPS.
...
O mercado é dominado por Gateways maduros e ultrarrápidos escritos em C ou Go, como Kong, Nginx ou AWS API Gateway. Exemplo declarativo da anatomia de uma rota (Kong YAML) barrando acesso de clientes que passam do limite (Rate Limiting) antes de onerar a aplicação:
```yaml
routes:
