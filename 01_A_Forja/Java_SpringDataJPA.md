...
*   **Como Funciona:** Você cria uma interface que herda de `JpaRepository<Entidade, ID>`. O Spring, através de reflexão e *proxies* dinâmicos, constrói a classe concreta na memória, provendo métodos como `save()`, `findById()`, `delete()` e inferindo queries puramente pela leitura do nome do método (ex: `findByNome()`).
