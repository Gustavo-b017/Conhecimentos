...
*   **Como Funciona:** Na programação tradicional, se a classe `A` precisa da classe `B`, a classe `A` faz um `new B()`. Com o IoC, o [[Java_SpringBoot|Spring Container]] inicializa todas as classes necessárias na inicialização da aplicação e as "injeta" nas classes que pedirem por elas. 
*   **O Problema que Resolve:** Destrói o altíssimo [[Arquitetura_Acoplamento|acoplamento]] gerado pelo operador `new`. O código torna-se flexível, modular e altamente testável, pois você não precisa saber *como* a dependência foi criada, apenas como usá-la.
...
public class UsuarioService {
    ...
    // A classe exige a dependência, mas não dá 'new'. O Spring injeta aqui (via [[Java_Autowired|Autowired]]).
