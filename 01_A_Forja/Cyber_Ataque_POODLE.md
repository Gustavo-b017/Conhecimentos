...
POODLE (Padding Oracle On Downgraded Legacy Encryption) é uma vulnerabilidade de rebaixamento fatal que explora o design falho de servidores na Camada de Apresentação (Camada 6 do modelo OSI) para forçar o uso do obsoleto protocolo SSL 3.0 e decifrar comunicações seguras.

#### 2. A Desconstrução (Mecânica e Pontos de Falha)
- **Como Funciona:** Se a vítima e o servidor preferem o [[Rede_TLS]], mas ainda mantêm o suporte para SSL 3.0 como "legado de segurança", o atacante interfere no *Handshake* (MITM). Ele bloqueia as tentativas de usar TLS, forçando ambas as partes a rebaixarem (Downgrade) a conexão para o fraco SSL 3.0. Depois, ele ataca a fraqueza no sistema de preenchimento (padding) do bloco de cifra (CBC), vazando bytes em texto claro (como cookies de sessão) através da observação dos erros matemáticos.
...
O [[Cyber_Ataque_POODLE]] é o equivalente a **forçar o exército a lutar de espadas**. O seu computador tem um tanque de guerra blindado (TLS 1.2), mas guarda uma espada enferrujada (SSL 3.0) no porão por "questões de compatibilidade" com aliados velhos. O atacante (em um ataque de [[Cyber_Man_In_The_Middle]]) corta a gasolina do tanque de guerra, e como o sistema é burro, ele automaticamente diz: "Ok, o tanque quebrou, vamos usar a espada". Assim que você saca a espada, o atacante a quebra em pedaços e rouba o seu colar (os cookies de acesso).
