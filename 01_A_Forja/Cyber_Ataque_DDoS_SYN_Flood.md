...
*   **Como Funciona:** O atacante envia uma avalanche de pacotes com a [[Rede_TCP_Flags_e_Header|flag SYN]] ativada. O servidor alvo responde com SYN-ACK e reserva recursos (RAM) para a conexão, aguardando o ACK final do cliente. Como o atacante forjou o IP de origem ([[Cyber_Spoofing]]) ou simplesmente ignora a resposta, o ACK nunca chega. O servidor fica com milhares de conexões presas no estado "half-open" (semi-abertas).
