...
- **Como Funciona:** O atacante frauda ativamente a rede (geralmente usando o frágil [[Rede_ARP]] Spoofing ou injetando-se em sessões de roteamento e serviços desconfigurados). Ele engana a vítima dizendo "Eu sou o servidor", e engana o servidor dizendo "Eu sou a vítima". O tráfego de ambos passa pelo computador do invasor antes de ir para o destino.
- **O Problema que Resolve:** Para o atacante, eleva a escuta clandestina ([[Cyber_Eavesdropping]]) para manipulação destrutiva. Ele pode degradar uma conexão criptografada (SSL Stripping), injetar links de Phishing em tempo real ou reescrever transações financeiras.
- **Visão Sênior (Vulnerabilidades/Escala):** Protocolos modernos em Nuvem mitigam esse ataque forçando TLS 1.3 ponta a ponta e certificados HSTS. Contudo, no ambiente local de uma empresa, se o switch central não possuir inspeções agressivas de portas (DAI), e não existir a cultura do [[Cyber_Zero_Trust]] que obrigue autenticação contínua mesmo dentro da [[Rede_LAN]], o atacante ganha o controle total das sessões.
