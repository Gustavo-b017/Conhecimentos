...
*   **Visão Sênior (Vulnerabilidades/Escala):** O vetor de ataque número um em ambientes de nuvem não é a quebra de criptografia matemática, mas o **Misconfiguration** (Erro de Configuração). Um desenvolvedor com pressa pode alterar a permissão de um *Bucket S3* de "Privado" para "Público" para facilitar um teste e esquecer de reverter. O resultado? O banco de dados de 50 milhões de clientes da empresa fica acessível por qualquer pessoa na internet sem exigir senha, causando um vazamento massivo ([[Cyber_DLP]]) em segundos.
