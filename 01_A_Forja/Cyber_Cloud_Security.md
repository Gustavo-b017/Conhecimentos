...
*   **Como Funciona:** Transfere-se a barreira física tradicional ([[Rede_Firewall]] físico e cabos) para configurações lógicas gerenciadas via painéis web (Software Defined Networking - SDN). Baseia-se no "Modelo de Responsabilidade Compartilhada": o provedor (AWS, Azure) protege a segurança **DA** nuvem (o concreto do data center, refrigeração, falha do disco rígido), enquanto o cliente é responsável pela segurança **NA** nuvem (quem tem a senha do servidor, o [[Rede_Firewall]] virtual, a criptografia do dado).
...
*   **Visão Sênior (Vulnerabilidades/Escala):** O vetor de ataque número um em ambientes de nuvem não é a quebra de criptografia matemática, mas o **Misconfiguration** (Erro de Configuração). Um desenvolvedor com pressa pode alterar a permissão de um *Bucket S3* de "Privado" para "Público" para facilitar um teste e esquecer de reverter. O resultado? O banco de dados de 50 milhões de clientes da empresa fica acessível por qualquer pessoa na internet sem exigir senha, causando um vazamento massivo em segundos.
