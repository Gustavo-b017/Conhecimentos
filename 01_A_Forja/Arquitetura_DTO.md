...
*   **O Problema que Resolve:** Impede dois problemas letais corporativos:
    ...
    2.  *Mass Assignment:* Um atacante intercepta o JSON do Frontend, insere o campo malicioso `"admin": true` no meio, e como a sua API recebe a Entidade direto no banco, ele sobrescreve a tabela interna ganhando acesso. O DTO barra campos não esperados.
...
O [[Design_DTO]] é a **Bandeja de Plástico da Refeição do Hospital**. O cozinheiro (Camada de Dados) faz a comida na panela de aço gigante (A Entidade mapeada no Banco). Você não envia a panela inteira pelo corredor até o quarto do paciente (A Interface Web); a panela é pesada e perigosa (Over-fetching e Falta de Segurança). O enfermeiro (O Controlador) extrai apenas a porção correta do purê da panela, a deposita em uma bandeja descartável de plástico leve (O DTO) e a entrega.
