...
*   **Como Funciona:** O ataque opera em diferentes camadas. Pode atacar a Camada 4 com *[[Cyber_Ataque_DDoS_SYN_Flood|SYN Floods]]* para esgotar a memória de portas abertas em um servidor (half-open attack). Também pode abusar da falta de estado do [[Rede_UDP]] na Camada 3/4, forjando o IP da vítima e pedindo dados para servidores externos (como [[Rede_DNS|DNS]] ou [[Rede_NTP|NTP]]), que enviam respostas gigantescas de volta para a vítima (Ataque de Amplificação e Reflexão).
*   **O Problema que Causa:** Fere fatalmente o pilar da Disponibilidade da [[Cyber_Triade_CID|Tríade de Segurança (CID)]], causando indisponibilidade de serviços web e prejuízos financeiros massivos.
*   **Visão Sênior (Vulnerabilidades/Escala):** Atacantes modernos raramente usam as próprias máquinas; eles controlam *[[Cyber_Botnet|Botnets]]* gigantescas compostas por dispositivos de [[Cyber_IoT_Security|IoT]] infectados (como a botnet Mirai) para gerar tráfego. A mitigação local via [[Rede_Firewall|firewall]] corporativo é inútil contra ataques volumétricos; exige-se a filtragem e dispersão na nuvem com provedores como a Cloudflare ou [[Cloud_AWS|AWS]].
