...
*   **Como Funciona:** Ele opera "out-of-band" (fora da linha principal de tráfego). O switch de rede copia todo o tráfego (via porta SPAN/Mirror) e entrega ao IDS. Ele compara os pacotes com um banco de dados de assinaturas (como um antivírus de rede) ou analisa desvios de tráfego baseados em heurística (ex: um PC do RH que de repente começa a escanear IPs do banco de dados).
*   **O Problema que Resolve:** O [[Rede_Firewall]] não enxerga movimentação lateral (uma máquina da LAN atacando outra máquina da LAN). O IDS resolve o problema da "cegueira interna", dando visibilidade sobre o que acontece dentro do perímetro.
...
O [[Cyber_IDS]] é como o **Sistema de Câmeras de Segurança e Alarme Sonoro de um banco**. Ele observa o saguão e, se alguém sacar uma arma, ele dispara a sirene e avisa a central. Mas **ele não tem a capacidade física de impedir o assalto** (não trava portas, não atira). Se o criminoso for rápido o suficiente e a polícia demorar, o dano será concretizado. Na TI, o alarme vai para a tela de uma plataforma de monitoramento contínuo (SIEM) para que um analista humano tome a decisão.
...
```text
# Regra do Snort: Alerta para tráfego TCP externo tentando atingir a porta 23 da LAN
