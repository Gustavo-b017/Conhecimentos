...
*   **Como Funciona:** Baseado no conceito de "Desacoplamento Espacial e Temporal" orquestrado por um *Message Broker* (como [[Ferramenta_Kafka|Kafka]], [[Ferramenta_RabbitMQ|RabbitMQ]] ou [[Cloud_AWS_SNS|AWS SNS]]). O Serviço A (Vendas) cria um pedido e simplesmente atira no Broker a mensagem: "Pedido #123 Criado". Ele não fica esperando resposta. O Serviço B (Estoque) e o Serviço C (Envio de E-mail) assinam esse evento e reagem no seu próprio tempo de processamento.
...
*   **Visão Sênior (Vulnerabilidades/Escala):** Arquiteturas baseadas em eventos introduzem a *[[Arquitetura_Consistencia_Eventual|Consistência Eventual]]* (o dado vai estar correto em todos os bancos, eventualmente, mas não no mesmo milissegundo). Isso gera pânico no time de negócios ("Vendi, mas não atualizou a tela!"). Outro risco letal é a falha no consumo de eventos: se a mensagem falhar múltiplas vezes, o sistema exige uma [[Arquitetura_Dead_Letter_Queue]] configurada, ou o Broker perderá a mensagem (Data Loss) ou entrará em loop eterno travando a fila (Poison Pill).
