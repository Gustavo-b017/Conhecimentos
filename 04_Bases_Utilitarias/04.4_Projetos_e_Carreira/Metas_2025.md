---
tags: [status/arquivo, tipo/moc, contexto/carreira, afinidade/pendente]
---

- [x] Ir na academia pelo menos 4 vezes na semana. ([[Mindset_Disciplina]])
	- [x] Emagrece 7 Kg no mínimo 
	- [x] Secar pelo mesmo 20% da gordura corporal
	- [x]  Construir uma musculatura minimamente aparente 
		- [x] focar em desenvolver o ombro e lombar
	- [x] Ser flexível e resolver as dores no corpo, praticando yoga umas 2 vezes na semana
	- [x] Participar de no mínimo 2 aulas de uma arte marcial na semana 
- [ ] Faculdade:
	- [x] Ler e compreender estatística básica
	- [ ] Ler e compreender probabilidade 
	- [x] Desenvolver habilidades ligadas ao Linux ([[Infra_Linux_Comandos]] e [[Ferramenta_Rsync]])
	- [x] Começar a aprender Cyber Segurança ([[Cyber_Roadmap_Iniciante]] e [[Cyber_Monitoramento_Continuo]])
	- [x] Desenvolver a solução do Challenger
	- [x] Ter uma média de 8 pontos ou mais no boletim 
	- [x] Aprender a montar slide
	- [x] Fazer amigos em outras faculdades 
	- [x] Ir a pelo menos 4 festas universitárias
- [ ] Pessoal: 
	- [x] Ler e compreender os livros:
		- [ ] A arte da sedução 
		- [ ] As armas da persuasão ([[Leitura_Armas_da_Persuasao]] e [[Social_Persuasao]])
		- [ ] O príncipe ([[Leitura_Maquiavel]] e [[Mindset_Estrategia]])
		- [ ] Lethur
		- [ ] O livro dos 5 aneis
	- [x] Chegar no inglês intermediário ([[Idioma_Ingles_Vocab]] e [[Idioma_Ingles_Gramatica]])
	- [ ] Assistir a 2 séries completas em inglês
	- [x] Estudar economia básica ([[Financas_Macroeconomia]])
		- [ ] aprender sobre juros
		- [ ] termos técnicos
		- [ ] história da economia
		- [ ] bases econômicas 
		- [ ] como a economia funciona
		- [ ] tipos de economia 
		- [ ] aprender sobre o mercado financeiro
			- [ ] renda variável e renda fixa
			- [ ] FIIs
			- [ ] mercado imobiliário 
			- [ ] cyptomoeda
			- [ ] ETF
	- [x] Desenvolver mais o Obsidian
		- [x] Transcrever os ideias de pelo menos 1 livro para a [[01_A_Forja]]
		- [x] Popular com mais notas
		- [x] Melhorar a organização 
		- [x] Inserir notas úteis de de fácil acesso
		- [x] Colocar conteúdos de linguagem corporal 
	- [x] Beijar pelo menos 3 garotas diferentes ( ou arrumar uma namorada )
	- [ ] Começar o curso de leitura da Vencer Leitura 
		- [ ] convencer o papai a comprar
	- [x] Melhorar a potência energética ( e sexual )
	- [x] Fazer pelo menos 1 amigo verdadeiro 
	- [x] Ir em uma fonoaudióloga e melhorar a potência vocal e deixar mais grave
	- [x] Melhorar a voz!!
	- [x] Fazer um cabelo estilo old money
	- [x] Ir em encontros de inglês no mínimo 2 vezes no mês 
- [ ] Viagens:
	- [ ] Fazer um intercâmbio nesse ano
	- [x] chegar no Inglês intermediário 
	- [x] Planejar algumas viagens sozinho ou em dupla
	- [ ] Ir na caverna do diabo
	- [ ] Se der visitar Curitiba ( se o Cris estiver lá )
	- [x] Realizar uma viagem solo
	- [x] Visitar palmas 2 vezes no mínimo 
	- [ ] Pular de bungee jump ou ir na tirolesa
	- [x] Visitar outro país 
- [ ] Financeiro: 
	- [ ] Arrumar um estágio ( de preferência em um banco ou uma corretora )
	- [x] Ter no mínimo 10 mil reais investidos 
	- [x] Diversificar meu portfólio 
	- [ ] Ter uma fonte de renda
	- [ ] Se possível montar uma reserva de emergência 

## Janeiro

## Fevereiro 

## Março 

## Abril

## Maio

## Junho

## Julho

## Agosto

## Setembro

## Outubro 

## Novembro 

## Dezembro